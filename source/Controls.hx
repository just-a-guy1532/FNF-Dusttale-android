...
class Controls extends FlxActionSet
{
	...
	var _atk = new FlxActionDigital(Action.ATK);
	...
	public var ATK(get, never):Bool;

	inline function get_ATK()
		return _atk.check();
	...
	public function setVirtualPad(virtualPad:FlxVirtualPad, ?DPad:FlxDPadMode, ?Action:FlxActionMode) {
		...
		switch (Action)
		{
			...
			case E:
				inline forEachBound(Control.ATK, (action, state) -> addbutton(action, virtualPad.buttonATK, state));
			case A_B:
				inline forEachBound(Control.ACCEPT, (action, state) -> addbutton(action, virtualPad.buttonA, state));
