...
class CustomControlsState extends MusicBeatSubstate
{
	...
	public function new()
	{
		...
		//pad
		_pad = new FlxVirtualPad(RIGHT_FULL, E);
		_pad.alpha = 0;
		...
		atk_text = new FlxText(200, 400, 0,"button ATK x:" + _pad.buttonATK.x +" y:" + _pad.buttonATK.y, 24);		
	...
	function trackbutton(touch:flixel.input.touch.FlxTouch){
		...
		if (buttonistouched){
			...
			if (_pad.buttonATK.justPressed) {
				if (curSelected != 3)
					changeSelection(0,3);

				movebutton(touch, _pad.buttonATK);
			}			
		}
	}
	...
	function setbuttontexts() {
		...
		atk_text.text = "Button A x:" + _pad.buttonATK.x +" y:" + _pad.buttonATK.y;		
	}
